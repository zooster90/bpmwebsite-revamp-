...
<div class="bt-an" style="display:flex;flex-direction:column;gap:20px;padding-bottom:40px;">
    ...
    <div style="background:linear-gradient(135deg,#0a1326 0%,#112240 60%,#1a365d 100%);border-radius:20px;padding:28px 32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;">
            ...
            <div style="background:rgba(34,197,94,.12);border:1px solid rgba(34,197,94,.3);border-radius:12px;padding:12px 20px;text-align:center;">
                <div style="font-size:1.8rem;font-weight:800;color:#4ade80;line-height:1;">{{ $s['active_now'] ?? 0 }}</div>
            ...
            <div style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:12px 20px;text-align:center;">
                <div style="font-size:1.8rem;font-weight:800;color:#fff;line-height:1;">{{ number_format($s['today_views'] ?? 0) }}</div>
            ...
            <div style="background:rgba(197,160,89,.12);border:1px solid rgba(197,160,89,.3);border-radius:12px;padding:12px 20px;text-align:center;">
                <div style="font-size:1.8rem;font-weight:800;color:#c5a059;line-height:1;">{{ number_format($s['month_sessions'] ?? 0) }}</div>
    ...
    <div class="grid-kpis">
        ...
        @endphp
        @foreach($kpis as $k)
        <div class="bt-card" style="padding:22px 24px;border-top:3px solid {{ $k['color'] }};">
            <div style="font-size:1.5rem;margin-bottom:8px;">{{ $k['icon'] }}</div>
            <div class="bt-label">{{ $k['label'] }}</div>
            <div class="bt-val" style="color:{{ $k['color'] }};margin-top:4px;">{{ $k['value'] }}</div>
            <div class="bt-sub">{{ $k['sub'] }}</div>
        </div>
        @endforeach
    </div>
    ...
    <div class="grid-breakdown">
        ...
        <div class="bt-card" style="padding:24px;">
            ...
            <div style="margin-bottom:10px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
                    <span style="font-size:.8rem;font-weight:600;color:#374151;">{{ $b['browser'] }}</span>
                    <span style="font-size:.8rem;font-weight:800;color:#0a1326;">{{ round(($b['count']/$totalBrowsers)*100) }}%</span>
                </div>
                <div class="bt-bar-track"><div class="bt-bar-fill" style="width:{{ round(($b['count']/$totalBrowsers)*100) }}%;background:{{ $bColors[$i] ?? '#94a3b8' }};"></div></div>
            </div>
            @endforeach
        </div>
        ...
        <div class="bt-card" style="padding:24px;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <div style="display:flex;align-items:center;gap:7px;">
                    <div style="width:10px;height:10px;border-radius:3px;background:{{ $dColors[$i] ?? '#94a3b8' }};flex-shrink:0;"></div>
                    <span style="font-size:.8rem;font-weight:600;color:#374151;">{{ $d['device'] }}</span>
                </div>
                <span style="font-size:.8rem;font-weight:800;color:#0a1326;">{{ round(($d['count']/$totalDevices)*100) }}%</span>
            </div>
            @endforeach
        </div>
        ...
        <div class="bt-card" style="padding:24px;">
            ...
            @foreach($osList as $i => $o)
            <div style="margin-bottom:10px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
                    <span style="font-size:.8rem;font-weight:600;color:#374151;">{{ $o['os'] }}</span>
                    <span style="font-size:.8rem;font-weight:800;color:#0a1326;">{{ $o['count'] }}</span>
                </div>
                <div class="bt-bar-track"><div class="bt-bar-fill" style="width:{{ round(($o['count']/$totalOS)*100) }}%;background:linear-gradient(90deg,#8b5cf6,#a78bfa);"></div></div>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="grid-pages">
            ...
            <div style="margin-bottom:12px;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                    ...
                    <span style="font-size:.78rem;font-weight:800;color:#3b82f6;white-space:nowrap;margin-left:8px;">
                        {{ number_format($p['views']) }} / {{ number_format($p['visitors']) }}
                    </span>
                </div>
                <div class="bt-bar-track">
                    <div class="bt-bar-fill" style="width:{{ round(($p['views']/$maxPageViews)*100) }}%;background:linear-gradient(90deg,#3b82f6,#60a5fa);"></div>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="bt-card" style="padding:24px;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                ...
                <span style="font-size:.82rem;font-weight:800;color:#8b5cf6;">{{ $r['count'] }}</span>
            </div>
            <div class="bt-bar-track">
                <div class="bt-bar-fill" style="width:{{ round(($r['count']/$maxReferrer)*100) }}%;background:linear-gradient(90deg,#8b5cf6,#a78bfa);"></div>
            </div>
            @empty
            ...
            <table style="width:100%;border-collapse:collapse;font-size:.82rem;">
                ...
                <tbody>
                    ...
                    <tr style="border-top:1px solid #f1f5f9;">
                        <td style="padding:10px 14px;font-weight:600;color:#1e293b;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $v['url'] }}</td>
                        <td style="padding:10px 14px;color:#374151;">
                            @if($v['device']==='Mobile') 📱 @elseif($v['device']==='Tablet') 🖥 @else 💻 @endif
                            {{ $v['device'] }}
                        </td>
                        <td style="padding:10px 14px;color:#374151;">{{ $v['browser'] }}</td>
                        <td style="padding:10px 14px;color:#374151;">{{ $v['os'] }}</td>
                        <td style="padding:10px 14px;font-weight:600;color:#8b5cf6;">
                            {{ $v['time_on'] > 0 ? ($v['time_on'] >= 60 ? floor($v['time_on']/60).'m '.($v['time_on']%60).'s' : $v['time_on'].'s') : '—' }}
                        </td>
                        <td style="padding:10px 14px;color:#94a3b8;white-space:nowrap;">{{ $v['time'] }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
(function(){
    ...
    const dailyLabels   = {!! json_encode(collect($dailyData)->pluck('date')) !!};
    const dailyViews    = {!! json_encode(collect($dailyData)->pluck('views')) !!};
    const dailyVisitors = {!! json_encode(collect($dailyData)->pluck('visitors')) !!};
    const hourLabels    = {!! json_encode(collect($hourlyData)->pluck('hour')) !!};
    const hourCounts    = {!! json_encode(collect($hourlyData)->pluck('count')) !!};
