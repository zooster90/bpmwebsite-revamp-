...
{{-- ═══════════════════════════════════════════════════════
     ROW 1 — GREETING BANNER (Full Width)
═══════════════════════════════════════════════════════ --}}
    ...
    <div class="bt-card-pad" style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:16px;">
        <div style="display:flex; align-items:center; gap:16px;">
            <div style="width:56px;height:56px;border-radius:16px;background:rgba(197,160,89,0.15);border:1px solid rgba(197,160,89,0.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                <span style="font-size:1.4rem;font-weight:800;color:#c5a059;">{{ $initials }}</span>
            </div>
            <div>
                <div style="font-size:1.5rem;font-weight:800;color:#fff;letter-spacing:-0.5px;">
                    {{ $greeting }}, <span style="color:#c5a059;">{{ explode(' ', $user->name)[0] }}</span>
                </div>
                <div style="font-size:0.85rem;color:rgba(255,255,255,0.55);margin-top:2px;font-weight:500;">
                    Builtech Command Center &mdash; {{ now()->format('l, d F Y') }}
                </div>
            </div>
        </div>
        <div style="display:flex;align-items:center;gap:24px;flex-wrap:wrap;">
            @foreach([['label'=>'Live Projects','val'=>$activeProjects,'color'=>'#c5a059'],['label'=>'Enquiries','val'=>$newInquiries,'color'=>'#f59e0b'],['label'=>'Applications','val'=>$totalApps,'color'=>'#22c55e']] as $s)
            <div style="text-align:center;">
                <div style="font-size:1.6rem;font-weight:800;color:{{ $s['color'] }};line-height:1;">{{ $s['val'] }}</div>
                <div style="font-size:0.65rem;font-weight:700;color:rgba(255,255,255,0.45);text-transform:uppercase;letter-spacing:0.08em;margin-top:3px;">{{ $s['label'] }}</div>
            </div>
            @if(!$loop->last)<div style="width:1px;height:32px;background:rgba(255,255,255,0.1);"></div>@endif
            @endforeach
            <div style="display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:10px;padding:8px 14px;font-size:0.72rem;font-weight:700;color:rgba(255,255,255,0.7);">
                ...
                <span id="bt-clock">{{ now()->format('H:i:s') }}</span>
            </div>
        </div>
    </div>
</div>
...
(function(){
    ...
    if (ctx && typeof Chart !== 'undefined') {
        new Chart(ctx, {
            type: 'line',
            data: {
                labels: {!! json_encode(collect(range(5,0))->map(fn($m) => now()->subMonths($m)->format('M'))->values()) !!},
                datasets: [{
                    data: {{ json_encode($sparkData) }},
                    borderColor: '#c5a059',
                    backgroundColor: (c) => {
