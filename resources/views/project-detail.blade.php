@extends('layouts.app')
@section('title', $project->meta_title ?: (($project->name ?? $project->title) . ' — Builtech Engineering Malaysia'))
@section('description', $project->meta_description ?: ($project->description ?? 'Explore the engineering excellence and construction details of this Builtech project — CIDB Grade G7 contractor in Malaysia.'))
...
<div class="bt-container">
    <header class="project-hero">
        <div class="reveal">
            <span class="project-tag">{{ $project->category?->name ?? 'General' }}</span>
            <h1 class="project-title">{{ $project->name ?? $project->title }}</h1>
        </div>

        <div class="spec-bar reveal">
            <div class="spec-item">
                <span class="spec-label"><i class="fas fa-map-marker-alt text-gold"></i> Location</span>
                <span class="spec-value">{{ $project->location ?? 'Malaysia' }}</span>
            </div>
            <div class="spec-item">
                <span class="spec-label"><i class="far fa-calendar-check text-gold"></i> Completion</span>
                <span class="spec-value">{{ $project->year ?? $project->completion_year ?? 'Ongoing' }}</span>
            </div>
            <div class="spec-item">
                <span class="spec-label"><i class="fas fa-tasks text-gold"></i> Sector</span>
                <span class="spec-value">{{ $project->category?->name ?? 'Infrastructure' }}</span>
            </div>
            @if($project->client)
                <div class="spec-item">
    ...
    <section class="gallery-section">
        ...
        <div class="gallery-grid">
            @foreach($images as $index => $img)
                <div class="gallery-item reveal {{ $index === 0 ? 'item-main' : 'item-sub' }}">
                    {{-- Use explicit href for GLightbox to work properly, but prevent right-click context menu --}}
                    <a href="{{ $img }}" data-type="image" data-gallery="project-gallery" class="glightbox" aria-label="View {{ $project->name ?? $project->title }} photo {{ $index + 1 }}" oncontextmenu="return false;">
                        <img src="{{ $img }}" alt="{{ ($project->name ?? $project->title) }} — Builtech Engineering Malaysia" loading="{{ $index === 0 ? 'eager' : 'lazy' }}" oncontextmenu="return false;">
                    </a>
                </div>
            @endforeach
        </div>
    ...
    <section class="related-section">
        <div class="bt-container">
            <div class="flex justify-between items-end reveal">
                ...
                <a href="{{ route('projects.index') }}" class="btn btn-outline border-white text-white hover:bg-white hover:text-navy px-8 py-3 text-sm">View All</a>
            </div>

            <div class="related-grid">
                @foreach($relatedProjects as $rel)
                    <a href="{{ $rel instanceof \App\Models\CurrentProject ? route('ongoing-projects.show', $rel->slug) : route('projects.show', $rel->slug) }}" class="related-card reveal">
                        <img src="{{ $rel->display_image ?? 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=800' }}" class="related-img" alt="{{ $rel->name ?? $rel->title }}">
                        <div class="related-body">
                            <span class="text-gold font-bold text-[10px] uppercase tracking-widest block mb-2">{{ $rel->category?->name ?? 'General' }}</span>
                            <h3 class="related-title">{{ $rel->name ?? $rel->title }}</h3>
                        </div>
                    </a>
                @endforeach
            </div>
...
@push('meta')
{{-- Schema.org: CreativeWork + ImageObject for Google copyright indexing --}}
@php
    $schemaImages = [];
...
{!! json_encode($creativeWorkSchema, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT) !!}
</script>
@endpush
