<?php $__env->startSection('title', 'Corporate Journal | Builtech Project Management'); ?>
        ...
        <div class="bt-container">
            <div class="flex flex-col lg:flex-row gap-20">
                ...
                <div class="flex-1 space-y-24">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(isset($articles) && $articles->count() > 0): ?>
                        ...
                        <article class="reveal group bg-white border border-gray-100 rounded-2xl shadow-2xl overflow-hidden transition-all duration-500 hover:translate-y-[-8px] hover:shadow-[0_20px_50px_rgba(10,25,47,0.12)]">
                            <!-- Image Container -->
                            <a href="<?php echo e(route('news.show', $featured->slug)); ?>" class="block relative aspect-[4/3] md:aspect-[21/9] overflow-hidden">
                                <img src="<?php echo e($featured->display_image ?? 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=1200'); ?>" 
                                ...
                                <div class="absolute top-6 left-6 z-20">
                                    <span class="bg-gold text-white text-[10px] font-extrabold uppercase tracking-widest px-4 py-2 rounded-sm shadow-md">
                                        <?php echo e($featured->category?->name ?? 'Featured Story'); ?>
                            ...
                            <div class="p-8 md:p-10 bg-white">
                                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 1.25rem;">
                                    <span style="font-size: 0.72rem; font-weight: 800; color: var(--gold); text-transform: uppercase; letter-spacing: 0.12em;"><?php echo e($featured->category?->name ?? 'Featured'); ?></span>
                                    <span style="width: 4px; height: 4px; background: #e2e8f0; border-radius: 50%; display: block;"></span>
                                    <span style="font-size: 0.85rem; color: var(--text-muted); font-weight: 600;"><?php echo e($featured->published_date ? \Carbon\Carbon::parse($featured->published_date)->format('d M Y') : $featured->created_at->format('d M Y')); ?></span>
                                </div>
                                ...
                                <h2 style="font-family: 'Oswald', sans-serif; font-size: clamp(1.8rem, 3.5vw, 2.5rem); font-weight: 800; color: var(--navy); margin-bottom: 1.25rem; line-height: 1.25;" class="group-hover:text-gold transition-colors duration-300">
                                    <a href="<?php echo e(route('news.show', $featured->slug)); ?>"><?php echo e($featured->title); ?></a>
                                </h2>
                                
                                <p style="font-size: 1.05rem; color: var(--text-body); line-height: 1.85; margin-bottom: 2rem;" class="line-clamp-3">
                                    <?php echo e(Str::limit(strip_tags($featured->excerpt ?? $featured->content), 250)); ?>

                                </p>
                                
                                <a href="<?php echo e(route('news.show', $featured->slug)); ?>" style="font-size: 0.8rem; font-weight: 800; color: var(--navy); text-transform: uppercase; letter-spacing: 0.25em; display: inline-flex; align-items: center; gap: 8px; border-bottom: 2px solid var(--gold); padding-bottom: 4px;" class="hover:text-gold hover:border-gold transition-colors">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-16">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $articles->skip(1); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $article): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                <article class="reveal group" data-delay="<?php echo e(($loop->index % 2) * 200); ?>">
                                    <a href="<?php echo e(route('news.show', $article->slug)); ?>" class="block aspect-[4/3] overflow-hidden rounded-xl mb-6 relative">
                                        <img src="<?php echo e($article->display_image ?? 'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?q=80&w=800'); ?>" 
                                        ...
                                        <div class="absolute top-3 right-3">
                                            <span style="font-size: 0.72rem; font-weight: 800; background: rgba(255,255,255,0.95); color: var(--navy); padding: 4px 12px; border-radius: 4px; text-transform: uppercase; letter-spacing: 0.1em;"><?php echo e($article->published_date ? \Carbon\Carbon::parse($article->published_date)->format('M Y') : $article->created_at->format('M Y')); ?></span>
                                        </div>
                                    </a>
                                    <div>
                                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 0.75rem;">
                                            <span style="font-size: 0.72rem; font-weight: 800; color: var(--gold); text-transform: uppercase; letter-spacing: 0.12em;"><?php echo e($article->category?->name ?? 'Update'); ?></span>
                                            <span style="width: 4px; height: 4px; background: #d1d5db; border-radius: 50%; display: block;"></span>
                                            <span style="font-size: 0.82rem; color: var(--text-muted);"><?php echo e($article->published_date ? \Carbon\Carbon::parse($article->published_date)->format('d M Y') : $article->created_at->format('d M Y')); ?></span>
                                        </div>
                                        <h3 style="font-family: 'Oswald', sans-serif; font-size: 1.4rem; font-weight: 700; color: var(--navy); margin-bottom: 1rem; line-height: 1.2;">
                                            <a href="<?php echo e(route('news.show', $article->slug)); ?>" class="group-hover:text-gold transition-colors"><?php echo e($article->title); ?></a>
                                        </h3>
                                        <a href="<?php echo e(route('news.show', $article->slug)); ?>" style="font-size: 0.78rem; font-weight: 800; color: var(--navy); text-transform: uppercase; letter-spacing: 0.2em; display: flex; align-items: center; gap: 8px;" class="hover:text-gold transition-colors">
                ...
                <aside class="w-full lg:w-96 space-y-12">
                    ...
                    <div class="bg-white border border-gray-100 p-8 rounded-xl reveal">
                        ...
                        <form action="<?php echo e(route('news.index')); ?>" method="GET" class="relative">
                    ...
                    <div class="bg-white border border-gray-100 p-8 rounded-xl reveal">
                        ...
                        <nav class="space-y-6">
                            <a href="<?php echo e(route('news.index')); ?>" 
                               class="flex items-center justify-between text-sm group <?php echo e(!request('category') ? 'text-navy font-black' : 'text-gray-400 hover:text-navy'); ?>">
                                <span class="flex items-center gap-4">
                                    <span class="w-2 h-2 rounded-full <?php echo e(!request('category') ? 'bg-gold shadow-[0_0_10px_rgba(197,160,89,0.5)]' : 'bg-gray-100'); ?> transition-all"></span>
                                    Full Archive
                                </span>
                            ...
                            ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $topicMap; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $label => $num): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                <a href="<?php echo e(route('news.index', ['category' => $label])); ?>" 
                                   class="flex items-center justify-between text-sm group <?php echo e(request('category') == $label ? 'text-navy font-black' : 'text-gray-400 hover:text-navy'); ?>">
                                    <span class="flex items-center gap-4">
                                        <span class="w-2 h-2 rounded-full <?php echo e(request('category') == $label ? 'bg-gold shadow-[0_0_10px_rgba(197,160,89,0.5)]' : 'bg-gray-100 group-hover:bg-gold/40'); ?> transition-all"></span>
                                        <?php echo e($label); ?>

                                    </span>
                                    <span class="text-[10px] font-bold opacity-30 group-hover:opacity-100 transition-opacity"><?php echo e($num); ?></span>
                                </a>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                        </nav>
                    </div>
                    ...
                    <div class="bg-navy p-12 rounded-sm shadow-2xl reveal relative overflow-hidden group">
                        ...
                        <div class="relative z-10 text-center">
                            ...
                            <a href="<?php echo e(route('contact')); ?>" class="btn-primary" style="width: 100%; justify-content: center;">Consult Our Engineers</a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\built\Herd\builtech-app\resources\views/news.blade.php ENDPATH**/ ?>